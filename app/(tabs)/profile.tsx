...
export default function ProfileScreen() {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#F7F4F7', paddingTop: insets.top + 20 }}>
      <ScrollView className="px-6" showsVerticalScrollIndicator={false}>
        <Animated.View entering={FadeInDown.delay(100).duration(600)} className="mb-8">
          <Text className="text-3xl font-poppins-bold text-dark">Profile</Text>
        </Animated.View>
        ...
        >
          <View className="w-20 h-20 rounded-full bg-primary/10 items-center justify-center mb-4">
            <Ionicons name="person" size={40} color="#42224A" />
          </View>
          <Text className="font-poppins-bold text-xl text-dark">User Account</Text>
          <Text className="font-poppins text-gray-400">Manage your preferences</Text>
        </Animated.View>
        ...
        <Animated.View entering={FadeInDown.delay(300).duration(600)}>
          {MENU_ITEMS.map((item, index) => (
            ...
            >
              <View className="w-10 h-10 rounded-xl bg-gray-50 items-center justify-center mr-4">
                <Ionicons name={item.icon as any} size={20} color="#42224A" />
              </View>
              <Text className="flex-1 font-poppins-medium text-dark">{item.label}</Text>
          ...
          >
            ...
            <Text className="flex-1 font-poppins-medium text-red-500">Logout</Text>
          </TouchableOpacity>
        </Animated.View>
      </ScrollView>
    </View>
